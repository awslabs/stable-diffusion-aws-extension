    ...
    for k in db_config:
        if type(db_config[k]) is str:
            if k == "model_dir":
                ...
                for concept in db_config[k]:
                    concept["instance_data_dir"] = os.path.basename(concept["instance_data_dir"])
                    concept["class_data_dir"] = os.path.basename(concept["class_data_dir"])
            else:
                db_config[k] = db_config[k].replace("dummy_local_model", model_name)
